...
var anchorReference=tabClickEvent.target;var activePaneId=anchorReference.getAttribute("href")||anchorReference.children[0].getAttribute("href");var activePane=document.querySelector(activePaneId);activePane.classList.add("active");var cm=this.getActiveTabCM();if(cm)
cm.refresh();};WLEditorTabsHandler.prototype.tabCloseIconClicked=function(tabCloseEvent){var closeIcon=tabCloseEvent.target;var closeTab=closeIcon.parentNode;var editPaneId=closeTab.children[0].getAttribute("href");var editPane=document.querySelector(editPaneId);var id=this.getNumFromId(editPaneId);editPane.remove();closeTab.remove();var localFileList=localStorage.getItem('wl.editor.localFileList');if(localFileList){localFileList=JSON.parse(localFileList);if(localFileList[id]){localStorage.removeItem('wl.editor.localFile.'+id);delete localFileList[id];localStorage.setItem('wl.editor.localFileList',JSON.stringify(localFileList));}
this.refresh_edit_buttons(Object.keys(localFileList).length>0);}
tabCloseEvent.preventDefault();};WLEditorTabsHandler.prototype.initialize=function(editor,gmcpHandler){var _this=this;this.wlEditor=editor;this.gmcpHandler=gmcpHandler;if(this.micromodal==null){this.micromodal=require('micromodal');this.micromodal.init();}
var myTabs=document.querySelectorAll("ul.nav-tabs > li");for(var i=0;i<myTabs.length;i++){myTabs[i].addEventListener('click',this.myTabClicks.bind(this));myTabs[i].querySelector('.ui-icon-close').addEventListener('click',this.tabCloseIconClicked.bind(this));}
